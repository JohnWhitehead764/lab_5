---
title: "Lab 5"
author: "Ashlan Simpson, John Whitehead, Tyler Christians"
date: "4/14/2021"
...
```{r}
Fatal <- filter(per, acc$INJ_SEV==4)
```
...
```{r}
#When output actually works replace this text with the answer to the question
ggplot(states, aes(x=LONGITUD, y=LATITUDE, fill=FATALS)) + geom_polygon(aes(group=COUNTY))
```
8) Is summer or winter more dangerous? Does this depend on states? Explore and explain.
```{r}
#When output actually works replace this text with the answer to the question
ggplot(states, aes(x=LONGITUD, y=LATITUDE, fill=WEATHER)) + geom_polygon(aes(group=group))
```
